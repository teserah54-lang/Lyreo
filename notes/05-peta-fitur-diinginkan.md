...
| 7 | Search songs, albums, artists, videos, playlists | **ADA** | `SearchFilter` lengkap (termasuk ARTISTS — `YouTubeRepository.musicSearch` → `BrowseScreen`); **pencarian lokal (2026-09-07, gelombang 7)**: `LocalMusicRepository.search()` (MediaStore SQL `LIKE` judul/artis/album + folder kustom SAF di-walk, hanya filter SEMUA/LAGU) → `SearchViewModel.localTracks` → `SearchScreen` seksi "DI PERANGKAT" | `ui/screens/search/{OnlineSearchScreen,OnlineSearchResult,LocalSearchScreen,SearchScreen}.kt`, `innertube/pages/{SearchPage,SearchSummaryPage,SearchSuggestionPage}.kt` | – (selesai) |
