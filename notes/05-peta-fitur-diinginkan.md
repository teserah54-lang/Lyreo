...
| 15 | Reorder songs in playlist or queue | **ADA** | playlist: `PlaylistDetailScreen` sudah punya tombol ↑/↓ → `PlaylistViewModel.move` → `LibraryRepository.moveInPlaylist` (posisi disimpan di Room); queue: `PlayerManager.moveItem` → tombol ↑/↓ + **geser-kiri hapus (SwipeToDismissBox), gulir otomatis ke lagu aktif, tombol acak, lembar 88% layar** (pola Meld `Queue.kt`, 2026-09-07). Belum drag-and-drop sungguhan | `extensions/QueueExt.kt`, `playback/queues/{Queue,ListQueue,YouTubePlaylistQueue,…}.kt`, `models/PersistQueue.kt` | M |
