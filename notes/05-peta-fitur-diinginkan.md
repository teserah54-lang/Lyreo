...
| 3 | Personalized quick picks | **ADA** | `ui/vm/ViewModels.kt` (`home.quickPicks`), `ui/screens/HomeScreen.kt` (~205–222), personalisasi rasa lewat `data/taste/TasteRepository.kt` + `MusicTextAnalyzer.kt`; tanpa riwayat → lagu charts YouTube Music (2026-09-07) | `innertube/pages/HomePage.kt`, `viewmodels/HomeViewModel.kt`, `models/SpotifyHomeSection.kt` | S (perkaya sumber) |
| 4 | Library management | **ADA (lagu/album/artis)** | `ui/screens/LibraryScreen.kt` (tab FAVORIT & LOKAL kini punya pengelompokan LAGU/ALBUM/ARTIS via `ui/components/GroupedTrackList.kt` + `LibraryGroupSelector`, selesai 2026-09-08), `data/LibraryRepository.kt`, `data/db/{Entities,Daos}.kt`, favorit/like tersebar di 19 berkas | `ui/screens/library/Library{Songs,Albums,Artists,Playlists,Mix,Podcasts}Screen.kt`, `playback/MediaLibrarySessionCallback.kt` | M (sisa: Mix/Podcast & sub-layar terpisah) |
...
| 7 | Search songs, albums, artists, videos, playlists | **ADA** | `SearchFilter` lengkap (termasuk ARTISTS — `YouTubeRepository.musicSearch` → `BrowseScreen`); **tab default kini SEMUA** (pola Meld `OnlineSearchResult`: ringkasan semua jenis, 2026-09-07); **pencarian lokal (2026-09-07, gelombang 7)**: `LocalMusicRepository.search()` (MediaStore SQL `LIKE` judul/artis/album + folder kustom SAF di-walk, hanya filter SEMUA/LAGU) → `SearchViewModel.localTracks` → `SearchScreen` seksi "DI PERANGKAT" | `ui/screens/search/{OnlineSearchScreen,OnlineSearchResult,LocalSearchScreen,SearchScreen}.kt`, `innertube/pages/{SearchPage,SearchSummaryPage,SearchSuggestionPage}.kt` | – (selesai) |
| 8 | Live lyrics + animasi transisi mengikuti lirik | **ADA** | `lyrics/{LyricsRepository,LyricsProvider,LrcLibProvider,KuGouProvider}.kt` = tangga provider (LRCLIB → KuGou, bisa dimatikan di Pengaturan); `ui/components/LyricsSheet.kt` = lirik hidup ala Metrolist; **lirik full-bleed** di area utama Now Playing — struktur Meld `Player.kt`: area `Box(weight 1f)` berisi artwork ⇄ lirik, kontrol selalu terlihat, kotak tidak pernah berubah ukuran (2026-09-08) (posisi dibaca langsung `PlayerManager.positionNow()` tiap 50 ms, auto-scroll memusatkan baris aktif 1500 ms, jeda 2 dtk saat pengguna menggulir, ketuk baris = seek, alpha+skala per baris, fading edge, koreksi offset ±10 dtk) + `ui/utils/FadingEdge.kt` | `ui/component/{Lyrics,LyricsLine,LyricsCommon,LyricsComponents,LyricsImageCard,LyricsBackgroundStyle}.kt`, `lyrics/{LyricsHelper,KuGouLyricsProvider,LrcLibLyricsProvider,BetterLyricsProvider,LyricsPlusProvider}.kt` + modul `kugou/`, `lrclib/`, `betterlyrics/`, `paxsenix/` | M (animasi) + M (penyedia lirik) |
...
| 11 | Import playlists | **ADA** | `YtPlaylistScreen` membuka playlist YouTube dari URL (`ytplaylist/{url}`) dan punya tombol SIMPAN → playlist lokal via `LibraryRepository`. **Impor berkas (m3u/pls) selesai 2026-09-07:** `data/PlaylistImporter.kt` (parser M3U `EXTINF` + PLS `FileN/TitleN/LengthN`; URL YouTube → track YouTube, path berkas → dicocokkan ke MediaStore/SAF via `LocalMusicRepository.findByFileName`) + baris \"Impor playlist\" di tab PLAYLIST `LibraryScreen` + dialog hasil | `ui/menu/ImportPlaylistDialog.kt` | – (selesai) |
...
| 14 | Local playlist management | **ADA** | `ui/screens/PlaylistDetailScreen.kt` (rename, hapus track, reorder ↑/↓, **hapus playlist** — selesai 2026-09-08: `deletePlaylistById` di `Daos.kt`/`LibraryRepository` + `PlaylistViewModel.delete` + dialog konfirmasi + tombol hapus di header), `data/db/{Entities,Daos}.kt` (Room; sampul = thumbnail item pertama via subquery `coverUrl`) | `ui/component/CreatePlaylistDialog.kt`, `db/entities/Playlist*.kt`, `ui/screens/playlist/*` | M (sisa: drag-and-drop, ganti sampul manual) |
| 15 | Reorder songs in playlist or queue | **ADA** | playlist: `PlaylistDetailScreen` sudah punya tombol ↑/↓ → `PlaylistViewModel.move` → `LibraryRepository.moveInPlaylist` (posisi disimpan di Room); queue: **drag-and-drop sungguhan** (`sh.calvin.reorderable`, pola Meld `Queue.kt`, 2026-09-08) + geser dua arah hapus + URUNGKAN + gulir otomatis + tombol acak + lembar 88% layar; `PlayerManager.moveItem`/`insertAt` | `extensions/QueueExt.kt`, `playback/queues/{Queue,ListQueue,YouTubePlaylistQueue,…}.kt`, `models/PersistQueue.kt` | M |
...
| 2 | ~~#13 tempo/pitch~~ **selesai di gelombang 3**; ~~#7 filter ARTISTS + pencarian lokal~~ **selesai di gelombang 7**; sisa: #18 fade sleep timer (S) | nilai terasa, usaha kecil, tanpa dependensi baru |
